...
interface FamilyMedicalHistorySectionProps {
...
const FamilyMedicalHistorySection = ({
  encounterForm,
  setEncounterForm,
}: FamilyMedicalHistorySectionProps) => {
  const FamilyMedicalHistoryData: DiseaseHistory[] =
    encounterForm.interview?.history?.familySpecific ?? [];

  const handleToggleCondition = (key: string, checked: boolean) => {
    if (checked) {
      // ✅ Add condition if not already exists
      const alreadyExists = FamilyMedicalHistoryData.some(
        (d) => d.diseaseCode === key
      );
      ...
      setEncounterForm((prev) => {
        ...
        if (!updated.interview?.history?.familySpecific) {
          updated.interview!.history!.familySpecific = [];
        }
        updated.interview!.history!.familySpecific.push(newCondition);
        return updated;
      });
    } else {
      // ❌ Remove condition
      setEncounterForm((prev) => {
        ...
        if (updated.interview?.history?.familySpecific) {
          updated.interview.history.familySpecific =
            updated.interview.history.familySpecific.filter(
              (d) => d.diseaseCode !== key
            );
        }
        return updated;
  ...
  const handleUpdateRemarks = (key: string, value: string) => {
    setEncounterForm((prev) => {
      ...
      if (updated.interview?.history?.familySpecific) {
        updated.interview.history.familySpecific =
          updated.interview.history.familySpecific.map((d) =>
            d.diseaseCode === key
    ...
    <div className="mb-8">
      ...
      <div className="space-y-4">
        {LIBRARY.MEDICAL_HISTORY.map((condition) => {
          const isChecked = FamilyMedicalHistoryData.some(
            (d) => d.diseaseCode === condition.key
          );
          const entry = FamilyMedicalHistoryData.find(
            (d) => d.diseaseCode === condition.key
          );
...
export default FamilyMedicalHistorySection;
